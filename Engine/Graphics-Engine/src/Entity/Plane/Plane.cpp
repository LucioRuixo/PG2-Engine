...
void Plane::initializeRenderingData()
{
	if (!Entity::planeRenderingDataInitialized)
	{
		...
		for (int i = 0; i < PLANE_VERTICES; i++)
		{
			int xIndex = i * VERTEX_LENGTH;
			vec3 position = vec3(vertices[xIndex], vertices[xIndex + 1], vertices[xIndex + 2]);
			planeVertexPositions.push_back(position);
		}

		Entity::planeRenderingDataInitialized = true;
	}
	else cout << "Can not initialize plane rendering data: data has already been initialized" << endl;
}

Plane::Plane(bool renderizable) : Entity(renderizable) {}

Plane::Plane(vec3 _color, bool renderizable) : Entity(_color, renderizable) {}

Plane::Plane(Material _material, bool renderizable) : Entity(_material, renderizable) {}

Plane::Plane(vec3 _color, Material _material, bool renderizable) : Entity(_color, _material, renderizable) {}
