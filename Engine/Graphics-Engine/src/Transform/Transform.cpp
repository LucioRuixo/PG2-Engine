...
void Transform::processRotation(float pitch, float yaw, float roll, bool matchingParentRotation)
{
	...
	if (matchingParentRotation)
	{
		...
		{
			pitch += oldParentRotation.x;
			yaw += oldParentRotation.y;
			roll += oldParentRotation.z;
		}

		parentRotationMatched = true;
	}
...
{
	if (pitch != 0.0f) processRotation(pitch, 0.0f, 0.0f, false);
	if (yaw   != 0.0f) processRotation( 0.0f,  yaw, 0.0f, false);
	if (roll  != 0.0f) processRotation( 0.0f, 0.0f, roll, false);
}
...
void Transform::setRotation(float pitch, float yaw, float roll)
{
	...
	bool matchingParentRotation = !parentRotationMatched;
	if (rotationX != 0.0f) processRotation(rotationX, 0.0f, 0.0f, matchingParentRotation);
	if (rotationY != 0.0f) processRotation(0.0f, rotationY, 0.0f, matchingParentRotation);
	if (rotationZ != 0.0f) processRotation(0.0f, 0.0f, rotationZ, matchingParentRotation);
}
...
void Transform::setParent(Transform* _parent)
{
	...
	if (parent)
	{
		...
		if (!_parent) oldParentRotation = parent->getGlobalRotation();
	}

	parent = _parent;
	parentRotationMatched = false;
}
